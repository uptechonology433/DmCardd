...
class ProductionReportDAO extends Connection
{
    ...
    public function getProductionReportFilterDatesInGeneralRedeUzeDAO(ProductionReportModel $productionReportModel): array
    {
        $statement = $this->pdo->prepare("SELECT total_rastreio,
         ...
         to_char(dt_expedicao, 'DD/MM/YYYY') AS dt_expedicao,
         nome_arquivo_proc,
        total_cartoes, status FROM view_redeuze_producao  where dt_expedicao BETWEEN :expedicaoinicial AND :expedicaofinal OR dt_processamento BETWEEN :datainicial AND :datafinal;");
